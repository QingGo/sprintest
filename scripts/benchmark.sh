...
start_time=$(date +%s.%N)
uv run pytest -s examples/test_ai_model.py > /dev/null 2>&1
end_time=$(date +%s.%N)
cold_duration=$(echo "$end_time - $start_time" | bc)
...
start_time=$(date +%s.%N)
uv run stest examples/test_ai_model.py > /dev/null 2>&1
end_time=$(date +%s.%N)
first_run_duration=$(echo "$end_time - $start_time" | bc)
...
uv run stest examples/test_ai_model.py > /dev/null 2>&1
end_time=$(date +%s.%N)
warm_duration=$(echo "$end_time - $start_time" | bc)
echo -e "${GREEN}Warm Start Duration: ${warm_duration}s${NC}"

# 5. Summary
speedup=$(echo "scale=1; $cold_duration / $warm_duration" | bc)
echo -e "\n${BLUE}=== Summary ===${NC}"
